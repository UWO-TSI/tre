export default function Socials() {
  return (
    <div className="flex gap-2">
      <a href="https://www.facebook.com/Childcan/" target="_blank" rel="noreferrer">
        <img
          src="data:image/png;base64,"
          alt="facebook"
        />
      </a>
      <a href="https://www.linkedin.com/in/childcan/" target="_blank" rel="noreferrer">
        <img
          src="data:image/png;base64,"
          alt="linkedin"
        />
      </a>
      <a href="https://www.instagram.com/childcan/" target="_blank" rel="noreferrer">
        <img
          src="data:image/png;base64,"
          alt="instagram-new"
        />
      </a>
    </div>
  );
}
